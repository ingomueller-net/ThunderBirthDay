 ...
 */
if (typeof getCalendarManager().getCalendarPref_ == "function") {
    var myGetCalendarPref = function (v1,v2)
            { return getCalendarManager().getCalendarPref_(v1,v2); };
    var mySetCalendarPref = function (v1,v2,v3)
            { return getCalendarManager().setCalendarPref_(v1,v2,v3); };
    var myDeleteCalendarPref = function (v1,v2)
            { return getCalendarManager().deleteCalendarPref_(v1,v2); };
} else {
    var myGetCalendarPref = function (v1,v2)
            { return getCalendarManager().getCalendarPref(v1,v2); };
    var mySetCalendarPref = function (v1,v2,v3)
            { return getCalendarManager().setCalendarPref(v1,v2,v3); };
    var myDeleteCalendarPref = function (v1,v2)
            { return getCalendarManager().deleteCalendarPref(v1,v2); };
}
...
//         ASSERT(this.mProperties.toSource() == "({})", "setProperty calls before id has been set!");
        ...
        function takeOverIfNotPresent(oldPref, newPref, dontDeleteOldPref) {
            var val = myGetCalendarPref(this_, oldPref);
            if (val !== null) {
                if (!dontDeleteOldPref) {
                    myDeleteCalendarPref(this_, oldPref);
                }
                if (myGetCalendarPref(this_, newPref) === null) {
                    mySetCalendarPref(this_, newPref, val);
                }
            }
        }
        ...
        if (ret === undefined) {
            ...
            if ((ret === null) && !calProviderBase.mTransientProperties[aName]) {
                if (this.id) {
                    ...
                    ret = myGetCalendarPref(this, aName);
                }
                if (ret !== null) {
...
//                     break;
                ...
                mySetCalendarPref(this, aName, v);
            }
    ...
    deleteProperty: function cPB_deleteProperty(aName) {
        ...
        myDeleteCalendarPref(this, aName);
    },
